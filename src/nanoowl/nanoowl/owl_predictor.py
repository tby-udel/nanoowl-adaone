...
class OwlPredictor(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.use_trt_image_encoder = image_encoder_engine is not None and self.device.type == "cuda"
        try:
            ...
            self.model = OwlViTForObjectDetection.from_pretrained(model_name).eval()
        ...
        self.box_bias = None
        if not self.use_trt_image_encoder:
    ...
    def encode_text(self, text: List[str]) -> OwlEncodeTextOutput:
        ...
        input_ids = text_input['input_ids'].to(self.text_device)
        attention_mask = text_input['attention_mask'].to(self.text_device)
    ...
    def decode(self, 
        ...
        query_embeds = text_output.text_embeds.to(image_class_embeds.device)
    ...
    def load_image_encoder_engine(engine_path: str, max_batch_size: int = 1):
        ...
        class Wrapper(torch.nn.Module):
            ...
            def _ensure_context(self):
                context = getattr(self.base_module, "context", None)
                if context is None:
                    context = self.base_module.engine.create_execution_context()
                    if context is None:
                        raise RuntimeError(
                            "Failed to create TensorRT execution context for NanoOWL image encoder"
                        )
                    self.base_module.context = context
